...
export const paginatedResult = (model) => {
	return async (req, res, next) => {
		...
		// }
		try {
			results.results = await model.find().sort({ createdAt: -1 }).limit(limit).skip(skip).exec();
...
export const commentAuthorization = (req, res, next) => {
	verifyToken(req, res, () => {
		...
		if (req.user.id === req.body.postUserId || req.user.id === req.body.postId || req.user.admin) {
			next();
		} else {
			return res.status(403).json("You're not allowed to do that!");
		}
