...
const UserHeader = () => {
	...
	const followings = currentUser?.followings?.find((item) => item === user?._id);
		...
		<>
			<Stack direction="row" ml={20}>
				...
				<Stack ml={5}>
					...
					{currentUser?.mobile === "" ? null : <span>Phone: {currentUser?.mobile}</span>}
					{currentUser?.address === "" ? null : <span>Address: {currentUser?.address}</span>}
				</Stack>
				{currentUser?._id === user?._id ? null : followings === undefined || followers === undefined ? (
					<Chip label="Stranger" color="error" icon={<People />} sx={{ ml: 20 }} />
				) : followings === followers &&
				  (currentUser?.followings?.length && currentUser?.followers?.length) > 0 ? (
					<Chip label="Friend" color="primary" icon={<People />} sx={{ ml: 20 }} />
				) : null}
				{currentUser?._id === user?._id && (
					<Button onClick={() => handleClickOpen("paper")} variant="contained" sx={{ ml: 20, height: 50 }}>
						Edit
					</Button>
				)}
